...
//    }
    ...
    @objc func keyboardWillHide(_ notification: NSNotification) {
        CoalitionofNorthernGroups.webView.setNeedsLayout()
    }
    
    func initWebView() {
        CoalitionofNorthernGroups.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(CoalitionofNorthernGroups.webView);
        CoalitionofNorthernGroups.webView.uiDelegate = self;
        CoalitionofNorthernGroups.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    @objc func loadRootUrl() {
        ...
        CoalitionofNorthernGroups.webView.load(URLRequest(url: launchUrl))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            CoalitionofNorthernGroups.webView.isHidden = false;
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                CoalitionofNorthernGroups.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(CoalitionofNorthernGroups.webView.estimatedProgress);
    ...
    deinit {
        CoalitionofNorthernGroups.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: CoalitionofNorthernGroups.webView)
        }
